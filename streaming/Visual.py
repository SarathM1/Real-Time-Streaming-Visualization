...
class Visual:
    def __init__(self, callbackFunc, running):
        self.text1 = Div(text="""<h1 style="color:blue">Visualization of Real Time Data Streaming</h1>""",
                         width=900, height=50)  # Text to be displayed at the top of the webpage
        ...
        self.hover = HoverTool(  # To show tooltip when the mouse hovers over the plot data
            tooltips=[
                ...
                # Show x and y coordinates of the plot data points
                ("(x,y)", "(@x, $y)")
            ]
        )
        # Set pan, zoom, etc., options for the plot
        self.tools = "pan,box_zoom,wheel_zoom,reset"
        self.plot_options = dict(plot_width=800, plot_height=200, tools=[
                                 self.hover, self.tools])  # Set plot width, height, and other plot options
        self.updateValue = True  # Internal state for updating of plots
        # Define various plots. Return handles for data source (self.source) and combined plot (self.pAll)
    ...
    def definePlot(self):
        ...
        # Link x-axis of first and second graph
        p2 = figure(**self.plot_options, x_range=p1.x_range,
                    title="Computed Value")
        p2.xaxis.axis_label = "Time (Discrete)"
        p2.yaxis.axis_label = "Computed Value"
        p2.extra_y_ranges = {"class": Range1d(
            start=-1, end=2)}  # Add a secondary y-axis
        # Name and place the secondary y-axis on the right vertical edge of the graph
        p2.add_layout(LinearAxis(y_range_name="class",
                                 axis_label="Classification"), 'right')
        # Define source data for all plots
        source = ColumnDataSource(data=dict(x=[0], y1=[0], y2=[0], y3=[0]))
        # Define graphs for each plot
        r1 = p1.line(x='x', y='y1', source=source, color="firebrick",
                     line_width=2)  # Line plot for sensor raw data
        # Circles to indicate data points in first graph
        r1a = p1.circle(x='x', y='y1', source=source,
                        color="firebrick", fill_color="white", size=8)
        r2 = p2.line(x='x', y='y2', source=source, color="indigo",
                     line_width=2)  # Line plot for computed values
        # Circles to indicate data points in second graph
        r2a = p2.circle(x='x', y='y2', source=source,
                        color="indigo", fill_color="white", size=8)
        # Overlay step plot for binary classes in the second graph
        r3 = p2.step(x='x', y='y3', source=source, color="green",
                     line_width=2, y_range_name="class")
        # Place legends outside the plot area for each data source
        legend = Legend(items=[("Sensor Data", [r1, r1a])], location=(5, 30))
        p1.add_layout(legend, 'right')
        # Plot line may be hidden by clicking the legend marker
        p1.legend.click_policy = "hide"
        legend = Legend(items=[("Computed Value", [r2, r2a]),
                               ("Classification", [r3])], location=(5, 30))
    ...
    def update(self, val):
        if self.updateValue:  # Update the plots only if the 'self.updateValue' is True
            ...
            # Increment the time step on the x-axis of the graphs
            newx = self.source.data['x'][-1] + 1
            newy1 = val  # Assign raw sensor data to be displayed in the first graph
            # Perform computation (i.e., moving average) on the sensor data and plot in the second graph
            newy2 = (val + self.prev_y1)/2
            self.prev_y1 = newy1
            # Classify the signal (i.e., binary calssification) at each time point and plot the results in the second graph
            newy3 = newy2 > 5
            # Construct the new values for all columns, and pass to stream
            new_data = dict(x=[newx], y1=[newy1], y2=[newy2], y3=[newy3])
            # Feed new data to the graphs and set the rollover period to be xx samples
            self.source.stream(new_data, rollover=20)

    def checkbox1Handler(self, attr, old, new):
        if 0 in list(new):  # Verify if the first checkbox is ticked currently
            # Perform actions if the first checkbox was not ticked previously, i.e., it changed state recently
